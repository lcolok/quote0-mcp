...
import { MonochromeOptimizer } from './monochrome-optimizer.js';
import { DEVICE_SCREEN_SIZE } from './types.js';
...
async function handlePreviewMode(args: string[]) {
  const imagePath = args[0];
  const outputDir = args[1] || './previews';
...
async function handleMonochromeMode(args: string[]) {
  const imagePath = args[0];
  const outputDir = args[1] || './mono-optimized';
