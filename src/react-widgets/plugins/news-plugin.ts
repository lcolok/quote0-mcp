...
class NewsDataProvider implements WidgetDataProvider<NewsData> {
  ...
  getSources(): string[] {
    return ['mock', 'rss', 'rss-llm', 'rss-enhanced', 'rss-ax', 'rss-ax-inspired', 'prompt-profile', 'ax-optimized', 'api'];
  }
  ...
  getSourceDescription(source: string): string {
    const descriptions: Record<string, string> = {
      ...
      'prompt-profile': '🧠 提示配置处理 - 版本化 few-shot 配置；质量由独立留出集验证',
  ...
  async getData(source: string, params: NewsDataParams): Promise<NewsData> {
    ...
    switch (source) {
      ...
      default:
        const supportedSources = ['mock', 'rss', 'rss-llm', 'rss-enhanced', 'rss-ax', 'rss-ax-inspired', 'prompt-profile', 'ax-optimized', 'api'];
        if (rssModule) {
          const rssSourceParam = rssModule.getSupportedParams().find(p => p.name === 'source');
