...
import {
  RENDERER_REVIEW_TARGETS,
  renderRendererComparison,
  resolveRendererReviewTarget,
} from './renderer-review-service.js';

const app = new Hono();
const postgres = getPostgresDatabase();

function serializeTarget(target: (typeof RENDERER_REVIEW_TARGETS)[number]) {
  return {
    id: target.id,
    kind: target.kind,
    ...
    governance: rendererGovernanceForTarget(target),
  };
}

function normalizeStoredReview(row: Record<string, any> | undefined) {
  if (!row) return null;
  return {
    ...row,
    choice: row.choice === 'adaptive' ? 'candidate' : row.choice,
    candidate_renderer: row.adaptive_renderer,
  };
}
...
async function getLatestReview(newsId: number, targetId: string) {
  const result = await postgres.query(
    ...
    [newsId, targetId, TRMNL_COMPARISON_KEY],
  );
  return normalizeStoredReview(result.rows[0]);
}

app.get('/api/review/renderers/targets', (c) => {
  return c.json({
    success: true,
    data: RENDERER_REVIEW_TARGETS.map(serializeTarget),
...
app.get('/api/review/renderers/governance', (c) => {
  return c.json({
    success: true,
    version: RENDERER_GOVERNANCE_VERSION,
    tracks: RENDERER_TRACKS,
    targets: RENDERER_REVIEW_TARGETS.map((target) => rendererGovernanceForTarget(target)),
    promotionGate: TRMNL_PROMOTION_GATE,
  });
});

app.get('/api/review/renderers/:id', async (c) => {
  ...
  const target = resolveRendererReviewTarget(requestedTargetId);
  ...
  try {
    ...
    const [comparison, review] = await Promise.all([
      renderRendererComparison(row, target),
    ...
    console.error('Renderer A/B 渲染失败:', error);
    return c.json({
      success: false,
      error: error instanceof Error ? error.message : 'Renderer A/B 渲染失败',
    }, 500);
  }
});

app.put('/api/review/renderers/:id/review', async (c) => {
  const newsId = Number.parseInt(c.req.param('id'), 10);
  if (!Number.isFinite(newsId) || newsId <= 0) {
    return c.json({ success: false, error: '无效的 review subject id' }, 400);
  }
  ...
  const target = resolveRendererReviewTarget(targetId);
  if (!targetId || target.id !== targetId) {
    return c.json({ success: false, error: '需要有效 targetId' }, 400);
  }
  if (body?.choice !== 'primary' && body?.choice !== 'candidate' && body?.choice !== 'tie') {
    return c.json({ success: false, error: 'choice 必须是 primary / candidate / tie' }, 400);
  }
  ...
  try {
    ...
    const storedChoice = body.choice === 'candidate' ? 'adaptive' : body.choice;
    const result = await postgres.query(
    ...
    return c.json({ success: true, data: normalizeStoredReview(result.rows[0]) });
  } catch (error) {
    console.error('保存 Renderer A/B 评审失败:', error);
    return c.json({
      success: false,
      error: error instanceof Error ? error.message : '保存 Renderer A/B 评审失败',
    }, 500);
  }
});
