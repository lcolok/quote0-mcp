...
export interface VersionMetadata {
  ...
  createdAt: string;
  createdBy: string;
  description: string; // 用户备注（可选，可修改）
  aiSummary?: string; // AI自动生成的总结陈词
  stats: {
    totalSamples: number;
    highQuality: number;
...
export class SnapshotManager {
  ...
  async createSnapshot(
    ...
    await fs.writeFile(
      path.join(versionDir, 'samples.json'),
      JSON.stringify(samples, null, 2)
    );

    await fs.writeFile(
      path.join(versionDir, 'source-mapping.json'),
   ...
   * 列出所有版本
   */
  async listVersions(): Promise<SnapshotVersion[]> {
    ...
    for (const entry of entries) {
      ...
      if (stat.isDirectory()) {
        ...
        try {
          ...
          const metadata: VersionMetadata = JSON.parse(metadataRaw);
   ...
   * 获取版本详细信息
   */
  async getVersionDetails(version: string): Promise<{
    ...
    const metadata = JSON.parse(
      await fs.readFile(path.join(targetVersion.path, 'metadata.json'), 'utf-8')
    );
    const samples = JSON.parse(
      await fs.readFile(path.join(targetVersion.path, 'samples.json'), 'utf-8')
    );
    const sourceMapping = JSON.parse(
      await fs.readFile(path.join(targetVersion.path, 'source-mapping.json'), 'utf-8')
    );
...
`;
    ...
    await fs.writeFile(
      path.join(this.baseDir, 'models', 'production', 'version.json'),
      JSON.stringify(versionPointer, null, 2)
    );
   ...
   * 比较两个版本的差异
   */
  async compareVersions(v1: string, v2: string): Promise<{
    added: number;
    removed: number;
    modified: number;
  ...
  private calculateStats(samples: TrainingSample[]) {
  ...
  private calculateSourceBreakdown(samples: TrainingSample[]): Record<string, number> {
   ...
   * 更新版本描述（用户备注）
   */
  async updateVersionDescription(version: string, newDescription: string): Promise<void> {
    const details = await this.getVersionDetails(version);
    ...
    // 写回文件
    await fs.writeFile(
      path.join(targetVersion.path, 'metadata.json'),
  ...
  async generateAISummary(
    ...
    summary += topSources.map(([source, count]) =>
      `${source} (${count}个, ${((count / stats.totalSamples) * 100).toFixed(1)}%)`
    ).join('、');
    summary += '。\n';
